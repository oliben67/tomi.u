    ...
    match scrutinee_ty {
        ...
        Ty::Adt(id, _) => {
            if let Some(enum_def) = registry.lookup_enum(*id) {
                check_enum(
                    arms,
                    &enum_def.variants.iter().map(|v| v.name.clone()).collect::<Vec<_>>(),
                )
            } else {
...
fn check_bool(arms: &[ast::MatchArm]) -> Option<Vec<String>> {
    ...
    for arm in arms {
        match &arm.pattern {
            ast::Pattern::Literal { value: ast::Expr::BoolLiteral { value: true, .. }, .. } => {
                has_true = true;
            }
            ast::Pattern::Literal {
...
fn check_enum(arms: &[ast::MatchArm], variant_names: &[String]) -> Option<Vec<String>> {
    let covered: Vec<String> =
        arms.iter().filter_map(|arm| extract_variant_name(&arm.pattern)).collect();

    let missing: Vec<String> =
        variant_names.iter().filter(|v| !covered.contains(v)).cloned().collect();
...
mod tests {
    ...
    fn dummy_arm(pattern: Pattern, body: Expr) -> MatchArm {
        MatchArm { pattern, guard: None, body, span: Span::DUMMY }
    }
    ...
    fn bool_pat(val: bool) -> Pattern {
        Pattern::Literal {
            value: Expr::BoolLiteral { value: val, span: Span::DUMMY },
            span: Span::DUMMY,
        }
    }
    ...
    fn unit_body() -> Expr {
        Expr::Tuple { elements: vec![], span: Span::DUMMY }
    }
    ...
    fn bool_both_covered() {
        let arms =
            vec![dummy_arm(bool_pat(true), unit_body()), dummy_arm(bool_pat(false), unit_body())];
    ...
    fn numeric_without_wildcard_not_exhaustive() {
        let arms = vec![dummy_arm(
            Pattern::Literal {
                value: Expr::IntLiteral { value: 0, suffix: None, span: Span::DUMMY },
                span: Span::DUMMY,
            },
            unit_body(),
        )];
