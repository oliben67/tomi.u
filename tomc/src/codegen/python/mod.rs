...
impl PythonBackend {
    ...
    fn generate_prelude(&mut self) {
        ...
        if self.needs_typing || self.needs_protocol {
            ...
            self.writer
                .writeln(&format!("from typing import {}", parts.join(", ")));
        }
        self.writer.blank_line();
    ...
    fn generate_import(&mut self, import: &Import) -> Result<(), CompileError> {
        ...
        if let Some(items) = &import.items {
            ...
            self.writer
                .writeln(&format!("from {} import {}", module, names.join(", ")));
        } else if let Some(alias) = &import.alias {
            self.writer
                .writeln(&format!("import {} as {}", module, alias.node));
        } else {
    ...
    fn generate_trait_def(&mut self, t: &Trait) -> Result<(), CompileError> {
        self.writer
            .write(&format!("class {}(Protocol)", t.name.node));
        ...
        if imp.trait_path.is_some() {
            ...
            if self.config.include_comments {
                if let Some(ref tp) = imp.trait_path {
                    ...
                    self.writer
                        .writeln(&format!("# impl {} for {}", trait_name, target_name));
                }
            }
        }
    ...
    fn generate_type_alias_item(&mut self, ta: &TypeAlias) -> Result<(), CompileError> {
        ...
        self.writer
            .writeln(&format!("{} = {}", ta.name.node, ty_str));
        Ok(())
    }
    ...
    fn generate_const_item(&mut self, c: &Const) -> Result<(), CompileError> {
        ...
        if let Some(ty) = &c.ty {
            ...
            self.writer
                .writeln(&format!("{}: {} = {}", c.name.node, ty_str, val_str));
        } else {
            self.writer
                .writeln(&format!("{} = {}", c.name.node, val_str));
        }
        Ok(())
    }
        ...
        if s.fields.is_empty() && s.methods.is_empty() {
            ...
            for field in &s.fields {
                ...
                if let Some(default) = &field.default {
                    let val = self.generate_expr(default)?;
                    self.writer
                        .writeln(&format!("{}: {} = {}", field.name.node, ty_str, val));
                } else {
                    self.writer
                        .writeln(&format!("{}: {}", field.name.node, ty_str));
                }
            }
    ...
    fn generate_enum(&mut self, e: &Enum) -> Result<String, CompileError> {
        ...
        if all_unit {
            ...
            for variant in &e.variants {
                self.writer
                    .writeln(&format!("{} = auto()", variant.name.node));
            }
            ...
            self.writer.indent();
            self.writer
                .writeln("\"\"\"Base class for sum type variants.\"\"\"");
